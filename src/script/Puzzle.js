...
var _Gridistribution = require('./lib/Gridistribution.es6');

var _Gridistribution2 = _interopRequireDefault(_Gridistribution);
...
var Puzzle = function () {
	...
	_createClass(Puzzle, [{
		...
		value: function init() {
			...
			loader.add([{ name: "shade", url: "images/shade.jpg" }, { name: "clipart", url: "images/clipart.png" }, { name: "pause", url: "images/pause@2x.png" }, { name: "play", url: "images/play@2x.png" }]).load(function () {
				...
				for (var i = 0; i < 24; ++i) {
					...
					var sprite = new PIXI.Sprite(rect.generateCanvasTexture());
					sprite.mask = mask;
					sprite.addChild(mask);
					var shell = new PIXI.Sprite(sprite.generateCanvasTexture(_this2.renderer));
		...
		value: function enter(picture) {
			...
			var afterLoad = function afterLoad() {
				...
				// 拼图的底片 
				_this3.negative = new PIXI.Sprite(_this3.base);
				_this3.negative.set({
					x: _this3.puzzle.x,
					y: _this3.puzzle.y,
				...
				_this3.hideLoading();
				// 拼图收缩
				TweenMax.fromTo(_this3.puzzle, 1, {
   ...
   */
			this.clipart = {};
			// 按照难度剪裁后的宽度 
			var width = this.imageWidth / (this.col - 80 / 300);
			...
			for (var row = 0; row < this.row; ++row) {
				...
				for (var col = 0; col < this.col; ++col) {
					...
					// 对底纹进行裁剪
					{
						...
						// 拼块
						clipart.sprite = new PIXI.Sprite(new PIXI.Texture(this.base, new PIXI.Rectangle(_x, _y, width, height)));
						// 被选中的拼块
		...
		value: function fit(clipart) {
			...
			// 左边拼块
			var leftClipart = index % this.col === 0 ? { lock: false } : this.cliparts[index - 1];
			// 右边拼块
			var rightClipart = index % this.col === this.col - 1 ? { lock: false } : this.cliparts[index + 1];
			// 上边拼块
			var upClipart = index < this.col ? { lock: false } : this.cliparts[index - this.col];
			// 下边的拼块
			var downClipart = index / this.col >> 0 === this.row - 1 ? { lock: false } : this.cliparts[index + this.col];
			// 容器
			var parent = null;
			// 左拼块存在
		...
		value: function displayShell() {
			...
			if (this.shellTimeline === undefined) {
				...
				var tls = rnd.map(function (_ref4, index) {
					...
					var tl = new TimelineLite();
					tl.fromTo(shell, .6, { rotation: 0, left: x, top: y, scaleX: 0, scaleY: 0, alpha: 0 }, { rotation: Math.PI, scaleX: 1, scaleY: 1, alpha: .5 }).to(shell, 1.2, { top: "+=560", left: (Math.random() > .5 ? "+" : "-") + "=80", rotation: 0, alpha: 0 });
					return tl;
				});
...
},{"./lib/Event.es6":327,"./lib/Gridistribution.es6":328,"./lib/timer.es6":329,"./lib/utils.es6":330,"babel-polyfill":1}],327:[function(require,module,exports){
"use strict";
...
exports.default = Gridistribution;
...
Object.defineProperties(proto, {
	...
	origin: {
		...
		set: function set(coord) {
			...
			// 未形变的左上角坐标
			var pointA = { x: this._left, y: this._top };
			// 形变后的中心坐标
